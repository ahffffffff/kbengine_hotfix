...
def onLoginAppReady():
	...
	loginapp已经准备好了
	"""
	KBEDebug.INFO_MSG('onLoginAppReady: bootstrapGroupIndex=%s, bootstrapGlobalIndex=%s' % \
...
def onLoginAppShutDown():
	...
	这个loginapp被关闭前的回调函数
	"""
	KBEDebug.INFO_MSG('onLoginAppShutDown()')
...
def onRequestLogin(loginName, password, clientType, datas):
	...
	"""
	KBEDebug.INFO_MSG('onRequestLogin() loginName=%s, clientType=%s' % (loginName, clientType))
...
def onLoginCallbackFromDB(loginName, accountName, errorno, datas):
	...
	"""
	KBEDebug.INFO_MSG('onLoginCallbackFromDB() loginName=%s, accountName=%s, errorno=%s' % (loginName, accountName, errorno))
...
def onCreateAccountCallbackFromDB(accountName, errorno, datas):
	...
	errorno: KBEngine.SERVER_ERR_*
	"""
	KBEDebug.INFO_MSG('onCreateAccountCallbackFromDB() accountName=%s, errorno=%s' % (accountName, errorno))
