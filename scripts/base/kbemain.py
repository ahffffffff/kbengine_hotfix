...
def onReadyForLogin(isBootstrap):
	...
	"""
	if isBootstrap == True:
		KBEngine.createEntityLocally('Hotfix_Ent')
	return 1.0
...
def onInit(isReload):
	...
	@type isReload: bool
	"""
	KBEDebug.INFO_MSG('onInit::isReload:%s' % isReload)
...
def onCellAppDeath(addr):
	...
	某个cellapp死亡
	"""
	KBEDebug.WARNING_MSG('onCellAppDeath: %s' % (str(addr)))
